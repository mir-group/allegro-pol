...
from ..nn import (
    ...
    ForceStressPolarizationOutput as ForceStressPolarizationOutput_Module,
)
...
@model_builder
def AllegroPolarizationModel(
    l_max: int,
    parity: bool = True,
    do_born_charge: bool = True,
    **kwargs,
):
    r"""Allegro polarization model that predicts energies, forces, polarization, Born charges, and polarizability (and stresses if cell is provided).
    ...
    Args:
        ...
        do_born_charge (bool): whether to compute Born charges and polarizability (default ``True``)
        electric_field_normalization (float): normalization factor for electric field (default ``1.0``)
        irreps_elec_field_sh (Union[int, str, o3.Irreps]): irreps for electric field spherical harmonics (default same as edge SH)
    """
    ...
    return ForceStressPolarizationOutput_Module(
        _AllegroPolarizationEnergyModel(
            irreps_edge_sh=irreps_edge_sh,
            tensor_track_allowed_irreps=tensor_track_allowed_irreps,
            **kwargs,
        ),
        do_born_charge=do_born_charge,
    )


@model_builder
def _AllegroPolarizationEnergyModel(
    r_max: float,
    ...
    # polarization-specific params
    electric_field_normalization: float = 1.0,
    irreps_elec_field_sh: Optional[Union[int, str, o3.Irreps]] = None,
):
    # Default electric field SH irreps to same as edge if not provided
    ...
    # === two-body tensor embedding with electric field ===
    tensor_embed = TwoBodySphericalHarmonicElectricFieldTensorEmbed(
        irreps_edge_sh=irreps_edge_sh,
        irreps_elec_field_sh=irreps_elec_field_sh,
        num_tensor_features=num_tensor_features,
        electric_field_normalization=electric_field_normalization,
        forward_weight_init=forward_normalize,
